        ...
        try:
            ...
            with process.oneshot():
                return {
                    ...
                    'num_fds': process.num_fds() if hasattr(
                        process,
                        'num_fds'
                    ) else None
                }
    ...
    async def run_with_timeout(self, cmd: list, timeout: int = 30) -> int:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "Process completed with return code: %s",
                    return_code
                )
                return return_code
            except subprocess.TimeoutExpired:
...
def setup_logging(log_path: Path):
    ...
    logger.info(
        "Logging initialized with output to console and file: %s",
        log_path
    )
                ...
                if schedule_type == 'recurring':
                    if pattern:
                        ...
                        if (
                            ...
                            db_manager.update_schedule_last_run(
                                schedule_id,
                                now
                            )
                            logger.info(
...
def check_node_and_run_npm_install(web_dir):
    ...
    try:
        try:
            ...
            with console.status(
                    "[bold green]Installing Tailwind CSS...",
                    spinner="dots"
            ):
...
def create_temp_sh(
    ...
    try:
        ...
        logging.error(
            "An error occurred while creating temp script: %s",
            e,
            exc_info=True
        )
        raise
            ...
            with tempfile.NamedTemporaryFile() as tmp_file:
                ...
                with tarfile.open(fileobj=tmp_file, mode='r:') as tar:
                    for member in tar.getmembers():
                        ...
                        is_safe_path = os.path.commonprefix([
                            abs_output_path,
...
class OutStream:
    ...
    def read_lines(self):
        ...
        finished_lines = [
            line.decode(errors='replace') for line in finished_lines
        ]
...
async def run_subprocess(command, log_file):
    """Enhanced run_subprocess function."""
        ...
        try:
            while True:
                ...
                if process.poll() is not None:
                    logger.info(
                        "Process terminated with return code: %s",
                        process.returncode
                    )
                    break
...
async def fpk_build(directory: Union[str, None], use_euxo: bool = False):
    ...
    console.print(f"flatpack_dir: {flatpack_dir}")
    console.print(f"flatpack_directory: {flatpack_directory}")
    ...
    if not custom_json_path.exists() or not custom_json_path.is_file():
        logger.error(
            "custom.json not found in %s. Build process canceled.",
            build_dir
        )
        raise FileNotFoundError(
...
def fpk_cache_unbox(directory_name: str):
    ...
    try:
        ...
        logger.error(
            "Failed to cache the directory name '%s': %s",
            directory_name,
            e
        )
...
def fpk_create(flatpack_name, repo_url=TEMPLATE_REPO_URL):
    ...
    try:
        ...
        logger.info(
            "Copied template files to flatpack directory: %s",
            flatpack_dir
        )
...
def fpk_set_secure_file_permissions(file_path):
    ...
    try:
        ...
        logger.error(
            "Failed to set secure file permissions for %s: %s",
            file_path,
            e
        )
...
def fpk_unbox(
    ...
    if directory_name is None or not isinstance(
            directory_name,
            str
    ...
    for proc_info in processes:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "PID %s didn't respond to SIGTERM, using SIGKILL",
                    pid
                )
                process.kill()
...
def setup_routes(fastapi_app):
    ...
    async def abort_build(
        ...
        async with abort_lock:
            try:
                ...
                logger.info(
                    "Starting build abort from status: %s",
                    initial_status
                )
                ...
                if process_manager and process_manager.is_active():
                    ...
                    for attempt in range(MAX_TERMINATION_ATTEMPTS):
                        try:
                            ...
                            logger.error(
                                "Termination attempt %s failed: %s",
                                attempt + 1, e
                            )
    ...
    async def load_file(
        ...
        try:
            ...
            logger.error(
                "Error decoding JSON from file: %s",
                sanitized_filename
            )
            raise HTTPException(
                status_code=400,
    ...
    async def delete_schedule_entry(
        ...
        try:
            ...
            logger.error(
                "An error occurred while deleting the schedule entry: %s",
                e
            )
            raise HTTPException(
                status_code=500,
        ...
        try:
            ...
            for mapping in mappings:
                try:
                    ...
                    logger.error(
                        "Unexpected error processing mapping: %s",
                        str(e)
                    )
                    continue
    ...
    async def validate_token(request: Request, api_token: str = Form(...)):
        ...
        if not stored_token:
            return JSONResponse(
                content={"message": "API token is not set."},
                status_code=200
            )
...
def fpk_cli_handle_run(args, session):
    ...
    if not args.input:
        console.print(
            "Please specify a flatpack for the run command.",
            style="bold red"
        )
        return
    ...
    if args.share:
        with console.status(
                "[bold green]Establishing ngrok ingress...",
                spinner="dots"
        ):
            try:
                if args.domain:
...
def fpk_cli_handle_unbox(args, session):
    ...
    if args.local and not is_fpk:
        ...
        if not local_directory_path.exists() or not local_directory_path.is_dir():
            logger.error(
                "Local directory does not exist: '%s'.",
                directory_name
            )
            console.print(
    ...
    try:
        ...
        if unbox_result:
            ...
            logger.info(
                "Unboxing of flatpack '%s' was aborted.",
                directory_name
            )
            console.print(
