...
Our platform uses [RSA keys](https://en.wikipedia.org/wiki/RSA_(cryptosystem)) to verify the authenticity and integrity of flatpacks (FPK), which may contain training code and data. Ensuring these packages have not been tampered with and originate from a trusted source is essential, given the high stakes involved in working with artificial intelligence.
